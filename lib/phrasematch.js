    ...
    function getSets(phrases) {
        ...
        for (var a = 0; a < phrases.length; a++) {
            ...
            for (i = 0; i < text.length; i++) {
                ...
                if (true) {//(relev === 0 || termidx === lastidx + 1) {
            ...
            if (allRelevs && allResults && relev === relevMap) {
                allResults.push(id);
                allRelevs[id] = {
                    count: count,
        ...
        if (allRelevs && allResults && relevMap >= 0.50){
            result = allResults;
            relevs = allRelevs;
        }
            ...
            for (var a = 0; a < queue.length; a++) {
                ...
                for (var i = 0; i < grids.length; i++) {
                    ...
                    if (features[feat]) {
                        ...
                        obj.relev = Math.max(obj.relev, relev.relev);
                        var newReason = obj.reason | relev.reason;
                        if (newReason != obj.reason) {
                            obj.reason = newReason;
                            obj.count += relev.count;
                        }
                    } else {
